...
const RegisterForm = ({ onSuccess }: { onSuccess: () => void }) => {
    ...
    const { register, handleSubmit, getValues, setError, formState: { errors, isSubmitting } } = useForm<RegisterFormValues>({
    ...
    const onSubmitForm = async (data: RegisterFormValues) => {
        ...
        try {
            ...
            setPending(code); setExpiry(Date.now() + CODE_TTL_SEC * 1000); setCountdown(CODE_TTL_SEC); setStep("verify");
        } catch {
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            setPending(code); setExpiry(Date.now() + CODE_TTL_SEC * 1000); setCountdown(CODE_TTL_SEC); setCodeError("");
        } catch { setCodeError("Помилка повторного надсилання. Спробуйте ще раз."); } finally { setSending(false); }
    };

    const handleCodeComplete = async (entered: string) => {
        setCodeError("");
        if (!expiry || Date.now() > expiry) { setCodeError("Код протермінований. Надішліть новий."); return; }
        if (entered !== pendingCode) { setCodeError("Невірний код. Спробуйте ще раз."); return; }
    ...
    if (step === "verify") {
        return (
            <div style={S.verifyStep}>
                ...
                <p style={S.verifyDesc}>Ми надіслали 6-значний код на <strong>{getValues().email}</strong>.<br />Введи його нижче.</p>
                ...
                <div style={S.verifyFooter}>
                    {countdown > 0 ? (
                        ...
                        <button className="verify-resend-btn" style={S.verifyResendBtn} onClick={handleResend} disabled={sending}>
                            {sending ? "Надсилання…" : "Надіслати код ще раз"}
                        </button>
                    )}
                    <button className="verify-back-btn" style={S.verifyBackBtn} onClick={() => setStep("form")}>← Назад до форми</button>
                </div>
            </div>
        );
    }
        ...
        <form style={S.form} onSubmit={handleSubmit(onSubmitForm)} noValidate>
            <div style={S.field}>
                ...
                <input id="register-name" type="text" autoComplete="name" style={S.input}
                    {...register("displayName", { required: "Введіть ім'я", minLength: { value: 2, message: "Мінімум 2 символи" } })} />
            ...
            <div style={S.field}>
                ...
                <input id="register-confirm" type="password" autoComplete="new-password" style={S.input}
                    {...register("confirmPassword", { required: "Повторіть пароль", validate: (v) => v === getValues("password") || "Паролі не збігаються" })} />
