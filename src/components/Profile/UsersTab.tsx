...
import { getAllProfiles } from "../../lib/profilesService";
...
const UsersTab = () => {
    ...
    if (loading) {
        return <div className="profile-settings-block"><p className="bonuses-empty"><i className="fa-solid fa-spinner fa-spin"></i><br />Завантаження...</p></div>;
    }

    if (users.length === 0) {
        return <div className="profile-settings-block"><p className="bonuses-empty"><i className="fa-solid fa-users-slash"></i><br />Користувачів ще немає</p></div>;
    }

    return (
        <div className="profile-settings-block">
            <h3 className="profile-settings-title">Всі користувачі</h3>
            <p className="profile-settings-desc">Список зареєстрованих акаунтів на платформі (оновлюється в реальному часі).</p>
