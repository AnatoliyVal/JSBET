...
const MOCK_BONUSES = [
    { id: 1,  title: "Фартова Конюшина: до 125 ФС у Supercharged Clovers", depositMin: "від 500 ₴", expiresAt: Date.now() + (1 * 24 * 60 * 60 + 18 * 3600 + 44 * 60 + 57) * 1000, options: [{ label: "75 ФС, Ставка: 2", sub: "від 500 ₴" }, { label: "75 ФС, Ставка: 5", sub: "від 1 000 ₴" }, { label: "100 ФС, Ставка: 8", sub: "від 2 000 ₴" }, { label: "125 ФС, Ставка: 20", sub: "від 5 000 ₴" }] },
    { id: 2,  title: "Фартові Монети: до 125 ФС у 9 Coins Stack", depositMin: "від 500 ₴", expiresAt: Date.now() + (1 * 24 * 60 * 60 + 18 * 3600 + 34 * 60 + 57) * 1000, options: [{ label: "75 ФС, Ставка: 2", sub: "від 500 ₴" }, { label: "95 ФС, Ставка: 4", sub: "від 1 000 ₴" }, { label: "100 ФС, Ставка: 8", sub: "від 2 000 ₴" }, { label: "125 ФС, Ставка: 20", sub: "від 5 000 ₴" }] },
];
...
const BonusesTab = () => {
    ...
    const handlePromo = () => {
        ...
        if (!code) return;
        if (code === "JSBET") { activateVip(); setPromoMsg({ type: "ok", text: "🎉 Вітаємо! Тебе підвищено до статусу VIP!" }); }
        else if (code === "RAINBOW") { useAuthStore.getState().activateRainbow(); setPromoMsg({ type: "ok", text: "🌈 Магія! Твій нік тепер переливається всіма кольорами веселки!" }); }
        else if (code === "NEW") { activateNewBadge(); setPromoMsg({ type: "ok", text: "🔥 Промокод активовано! Ти отримав статус NEW на 2 дні!" }); }
        else if (code === "WELCOME" || code === "BONUS") { setPromoMsg({ type: "ok", text: `✅ Промокод "${promo.trim()}" активовано! Бонус нараховано.` }); }
        else { setPromoMsg({ type: "err", text: "❌ Невірний промокод або термін його дії минув." }); }
        setPromo("");
    };
        ...
        <div style={S.tabWrap}>
                                        ...
                                        <div style={S.options}>
                                            {b.options.map((opt, i) => (
                                                <label key={i} style={S.option(selectedOpts[b.id] === i)}>
                                                    <input type="radio" onChange={() => setSelectedOpts((prev) => ({ ...prev, [b.id]: i }))} checked={selectedOpts[b.id] === i} style={{ display: "none" }} />
            ...
            <div style={S.promoRow}>
                ...
                <div style={S.promoWrap}>
                    <input type="text" style={S.promoInput} placeholder="Введи промокод" value={promo} onChange={(e) => setPromo(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handlePromo()} />
