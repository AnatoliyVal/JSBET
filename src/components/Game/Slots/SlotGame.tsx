...
import { S, KEYFRAMES } from "./SlotStyle";
...
const SlotGame = ({ game, onClose }: Props) => {
    ...
    useEffect(() => {
        if (!spinning && autoRef.current && balance >= totalBet) {
            const t = setTimeout(() => { if (autoRef.current) spin(); }, 700);
    ...
    return createPortal(
        <div style={S.overlay}>
            ...
            <div style={S.decor(500, 500, "radial-gradient(circle, #ff6eb4, transparent)", 0.32, { top: -150, left: -100 })} />
            <div style={S.decor(400, 400, "radial-gradient(circle, #cc44ff, transparent)", 0.32, { bottom: -100, right: -80 })} />
            <div style={S.decor(280, 280, "radial-gradient(circle, #ffdd00, transparent)", 0.18, { top: "35%", left: "42%" })} />
