...
const Section = ({ id, heading, count, link, linkText, children, isMobile }: {
    id?: string; heading: string; count: string; link?: string; linkText?: string; children: React.ReactNode; isMobile?: boolean;
}) => (
        ...
        <main>
            <div style={{ display: "block" }} id="page-igri">
                        ...
                        <div style={S.container}>
                            <div style={S.heroInner}>
                                ...
                                <p style={S.heroSubtitle}>
                                    Найкращі слоти, Live games та джекпоти від провідних провайдерів. Реєструйся та отримуй бонуси прямо зараз!
                                </p>
                                <div style={S.heroActions}>
                                    <Button variant="primary" onClick={() => openAuthModal("register")}>Почати грати</Button>
                ...
                <div style={S.container}>
                    <div style={S.categoryRow} aria-label="Фільтр за тематикою">
                        <ul style={S.categoryList} role="list">
                            <li>
                                <button style={S.categoryChip(selectedCategory === "Всі")} type="button" onClick={() => setSelectedCategory("Всі")}>
                                    Всі
                                </button>
                            </li>
                            {categories.map(cat => (
                                <CategoryButton key={cat} CategoryName={cat} isActive={selectedCategory === cat} onClick={() => setSelectedCategory(cat)} />
                            ))}
                        </ul>
                    </div>
                </div>
                ...
                {selectedCategory === "Всі" ? (
                    <>
                        <Section id="recommended-heading" heading="Рекомендовані для вас" count="6 ігор" isMobile={isMobile}>
                            <RandomGameList count={6} />
                        </Section>
                        <Section id="popular-heading" heading="Популярні" count="18 ігор" link="#" linkText="Всі ігри" isMobile={isMobile}>
                        ...
                        <Section id="new-heading" heading="Нові ігри" count="12 ігор" link="#" linkText="Всі нові" isMobile={isMobile}>
                        ...
                        <Section id="jackpot-heading" heading="Джекпоти" count="8 ігор" link="#" linkText="Всі джекпоти" isMobile={isMobile}>
                        ...
                        <Section id="fishing-heading" heading="Рибалка" count="8 ігор" link="#" linkText="Всі" isMobile={isMobile}>
                        ...
                        <div style={S.container}>
                            <div style={S.sectionHeader}>
                                <h2 id="filtered-heading" style={S.sectionTitle}>
                                    ...
                                    <span style={S.sectionCount}>· {GAMES_DB.filter(g => g.CategoryName === selectedCategory).length} ігор</span>
                                </h2>
                            </div>
                            ...
                            {GAMES_DB.filter(g => g.CategoryName === selectedCategory).length === 0 && (
                                <p style={{ color: "#a0a0a0", paddingTop: 20 }}>На жаль, у цій категорії поки немає ігор.</p>
                            )}
                        </div>
                    </section>
                )}
