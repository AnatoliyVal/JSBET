...
import AccauntTopUpModal from "./ModulePage/AccauntTopUp";
                                ...
                                <nav style={S.nav} aria-label="Меню профілю">
                                    {navItems.map((item) => (
                                        <button key={item.id} type="button"
                                                style={S.navLink(activeTab === item.id, false)}
                                                onClick={() => setActiveTab(item.id)}>
                                            <i className={`fa-solid ${item.icon}`} style={{width: 20}}/> {item.label}
                                        </button>
                                    ))}
                                    <button type="button" style={S.navLink(false, true)} onClick={() => {
                                        logout();
                                        navigate("/");
                                    }}>
                                ...
                                {activeTab === "settings" && (
                                    <>
                                        <div style={S.block}>
                                            ...
                                            <p style={S.desc}>Управляйте своєю особистою інформацією та налаштуваннями
                                                акаунта.</p>
                                            <form onSubmit={handleSave}>
                                                <div style={S.formRow(isMobile)}>
                                                    <div style={S.group}>
                                                        <label style={S.label}
                                                               htmlFor="user-displayname">Нікнейм</label>
                                                        <input type="text" id="user-displayname" style={S.input}
                                                               value={displayName}
                                                               onChange={(e) => setDisplayName(e.target.value)}/>
                                                    </div>
                                                    <div style={S.group}>
                                                        <label style={S.label} htmlFor="user-email">Email</label>
                                                        <input type="email" id="user-email" style={S.input}
                                                               value={user.email} readOnly/>
                                                    </div>
                                                </div>
                                                <div style={S.formRow(isMobile)}>
                                                    <div style={S.group}>
                                                        <label style={S.label} htmlFor="user-phone">Номер
                                                            телефону</label>
                                                        <input type="tel" id="user-phone" style={S.input}
                                                               placeholder="+380..." value={phone}
                                                               onChange={(e) => setPhone(e.target.value)}/>
                                                    </div>
                                                    <div style={S.group}>
                                                        <label style={S.label} htmlFor="user-dob">Дата
                                                            народження</label>
                                                        <input type="date" id="user-dob" style={S.input} value={dob}
                                                ...
                                                <div style={S.formRow(isMobile)}>
                                                    <div style={S.group}>
                                                        ...
                                                        <select id="user-country" style={S.select} value={country}
                                                ...
                                                <div style={S.actions}>
                                                    <Button type="submit"
                                                            variant="primary">{saved ? "✓ Збережено!" : "Зберегти зміни"}</Button>
                                        ...
                                        <div style={S.block}>
                                            ...
                                            <p style={S.desc}>Додайте нову банківську картку для поповнення рахунку та
                                                виведення коштів.</p>
                                            <form action="#" method="POST">
                                                <div style={{...S.group, marginBottom: 24}}>
                                                    <label style={S.label} htmlFor="cardName">Ім&apos;я на карті</label>
                                                    <input type="text" id="cardName" style={S.input}
                                                ...
                                                <div style={{...S.group, marginBottom: 24}}>
                                                    ...
                                                    <div style={{
                                                        ...
                                                        <i className="fa-regular fa-credit-card" style={{
                                                            position: "absolute",
                                                            left: 16,
                                                            color: "var(--color-text-muted)"
                                                        }}/>
                                                        <input type="text" id="cardNumber"
                                                               style={{...S.input, width: "100%", paddingLeft: 44}}
                                                ...
                                                <div style={S.formRow(isMobile)}>
                                                    <div style={S.group}>
                                                        ...
                                                        <input type="text" id="expDate" style={S.input}
                                                               placeholder="MM/YY" maxLength={5} required/>
                                                    </div>
                                                    <div style={S.group}>
                                                        <label style={S.label} htmlFor="cvv">CVV</label>
                                                        <input type="password" id="cvv" style={S.input}
                                    ...
                                    <div style={S.block}>
                                        ...
                                        <div style={{display: "flex", flexDirection: "column", gap: 24}}>
                                            <div style={S.visualBlock}>
                                                <h4 style={S.visualTitle}><i className="fa-solid fa-palette"
                                                                             style={{color: "var(--color-gold)"}}/> Переливання
                                                    ніка</h4>
                                                <p style={S.visualDesc}>Активуйте магічний ефект веселки для вашого
                                                    нікнейму. Це бачитимуть всі користувачі.</p>
                                                <label style={{
                                                    display: "flex",
                                                    alignItems: "center",
                                                    gap: 10,
                                                    ...
                                                    <input type="checkbox" checked={user.rainbowActive}
                                            ...
                                            {((user.badges && user.badges.length > 0) || (user.isNewUntil && Date.now() < user.isNewUntil)) && (
                                                <div style={S.visualBlock}>
                                                    <h4 style={S.visualTitle}><i className="fa-solid fa-eye"
                                                                                 style={{color: "var(--color-gold)"}}/> Видимість
                                                        плашок</h4>
                                                    <p style={S.visualDesc}>Виберіть, які статуси та емблеми будуть
                                                        відображатися поруч із вашим іменем.</p>
                                                    <div style={{display: "flex", flexWrap: "wrap", gap: 10}}>
                                                        {user.badges?.map(b => (
                                                            <button key={b}
                                                                    onClick={() => useAuthStore.getState().toggleBadgeVisibility(b)}
                                                                    style={S.visualBtn(user.hiddenBadges?.includes(b) ?? false)}>
                                                                <i className={user.hiddenBadges?.includes(b) ? "fa-solid fa-eye-slash" : "fa-solid fa-eye"}
                                                                   style={{marginRight: 6}}/> {b}
                                                            </button>
                                                        ))}
                                                        {user.isNewUntil && Date.now() < user.isNewUntil && (
                                                            <button
                                                                onClick={() => useAuthStore.getState().toggleBadgeVisibility("NEW")}
                                                                style={S.visualBtn(user.hiddenBadges?.includes("NEW") ?? false)}>
                                                                <i className={user.hiddenBadges?.includes("NEW") ? "fa-solid fa-eye-slash" : "fa-solid fa-eye"}
                                                                   style={{marginRight: 6}}/> NEW
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
